...
**Status:** Phase 0 complete. Playable, scored, 1362 tests, **zero third-party
...
has hit points and you have a bounded pool of repairs, and a repair hands the
boss back health in proportion to how wrong the code was, so only a flawless
fight puts it down. Scoring the fight on the three axes is the last piece.
...
| `daily` | Today's challenge — the same level and variant for everyone, not adapted to you (`--date` for a past one, `--show` to name it) |
...
| `boss <id> --ability <key>` | Equip an ability for the fight — each has a stated cost, and a full loadout still loses |
...
├── keys.py           Byte stream → key events, including bracketed paste
├── editing.py        Text buffer, cursor, undo — no terminal, fully headless
...
tests/                1362 tests, stdlib unittest
...
```bash
python3 -m unittest discover -s tests     # 1.5-2.5 min; pin VIBECODER_SANDBOX=bwrap for ~40s
